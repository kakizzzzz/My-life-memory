...
`research_memory_context` is the preferred action for natural-language questions. It builds a structured query plan, applies explicit public-place and date constraints, resolves user-relative anchors only from first-person evidence in the authenticated archive, and then matches requested events, targets, or nearby routes. Candidate notes remain coordinate-free and separate from evidence, ambiguous anchors are not chosen by recency, and the latest saved memory is never presented as the user's current location. A mandatory answer boundary requires an evidence-only answer, an explicit no-answer, or a request for disambiguation; unresolved responses do not expose coordinate, route, or photo answer paths. Confidence bands are heuristic retrieval labels rather than calibrated probabilities.

When deterministic evidence is insufficient, semantic review uses a staged same-tool workflow. The first unresolved response withholds candidate text; a capable host may explicitly request one bounded candidate page, use its existing conversation model to compare aliases, nicknames, paraphrases, and implicit descriptions, and then resubmit exact passages as `supports`, `uncertain`, or `rejects`. The server revalidates the authenticated note, exact quote, relation, negation, exclusion rules, and target linkage before allowing evidence or coordinates. A merely plausible semantic link becomes a clarification question, not a memory fact. Clients that cannot continue the workflow must report insufficient verified evidence rather than guess. My Life Memory contains no model runtime, calls no model API, needs no model key, and creates no model-inference bill. `search_memories` keeps exact substring search for compatibility, but an empty literal result is automatically retried through the same contextual research flow so less capable clients do not stop at a missing phrase.
