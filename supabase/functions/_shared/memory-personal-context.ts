...
const explicitHomeIdentityPattern = /(?:这里|這裡|这儿|這兒|此处|此處).{0,12}(?:是|就是).{0,8}(?:我家|我的家|我的住处|我的住處|我的住所|我住的(?:房子|地方|公寓))|(?:我家|我的家|我的住处|我的住處|我的住所).{0,12}(?:在这里|在這裡|就是这里|就是這裡)|\bthis\s+is\s+my\s+(?:home|house|residence)\b|\bthis\s+is\s+where\s+i\s+live\b|(?:ここ|ここが).{0,8}(?:自宅|私の家)|(?:여기|이곳).{0,8}(?:우리 집|내 집)/iu;
const temporaryResidencePattern = /(?:住院|入院|出院|病房|留院|治疗期间|治療期間|康复期间|康復期間|暂住|暫住|临时住|臨時住|借住|入住(?:酒店|旅馆|旅館|宾馆|賓館|民宿|青旅)|(?:住|居住)了\s*(?:[一二三四五六七八九十百两兩\d]+|几|幾)\s*(?:天|晚|周|星期|个月|個月)|(?:康复医院|康復醫院|康复中心|康復中心|医院|醫院|诊所|診所|疗养院|療養院).{0,24}(?:住|居住|治疗|治療|康复|康復|留院)|(?:住|居住|治疗|治療|康复|康復|留院).{0,24}(?:康复医院|康復醫院|康复中心|康復中心|医院|醫院|诊所|診所|疗养院|療養院)|\b(?:inpatient|hospitali[sz]ed|hospital\s+stay|rehab(?:ilitation)?\s+stay|temporary\s+(?:stay|lodging)|stayed?\s+(?:here\s+)?for\s+\d+\s+(?:days?|nights?|weeks?|months?))\b|(?:入院|病室|一時滞在|病院.{0,18}(?:滞在|入院))|(?:입원|병실|임시\s*(?:숙박|거주)|병원.{0,18}(?:머물|입원)))/iu;
const incidentalWorkPattern = /(?:参观|參觀|游览|遊覽|路过|路過|经过|經過|面试|面試|应聘|應聘|办事|辦事|拜访|拜訪).{0,18}(?:公司|办公室|辦公室|单位|單位|工厂|工廠)|\b(?:visited?|toured?|passed|interviewed)\b.{0,24}\b(?:company|office|workplace|factory)\b|(?:会社|職場|事務所).{0,12}(?:見学|訪問|通過)|(?:회사|직장|사무실).{0,12}(?:방문|구경|지나)/iu;
const incidentalStudyPattern = /(?:参观|參觀|游览|遊覽|路过|路過|经过|經過|开放日|開放日).{0,18}(?:学校|學校|校园|校園|大学|大學|教室|图书馆|圖書館)|\b(?:visited?|toured?|passed)\b.{0,24}\b(?:school|campus|university|classroom|library)\b|(?:学校|大学|教室|図書館).{0,12}(?:見学|訪問|通過)|(?:학교|대학교|교실|도서관).{0,12}(?:방문|구경|지나)/iu;

const hardIdentityExclusion = (relation: PersonalAnchorRelation, text: string) => {
  if (relation === 'home' && temporaryResidencePattern.test(text) && !explicitHomeIdentityPattern.test(text)) {
    return 'temporary-or-institutional-stay';
  }
  if (relation === 'work' && incidentalWorkPattern.test(text)) return 'incidental-workplace-visit';
  if (relation === 'study' && incidentalStudyPattern.test(text)) return 'incidental-study-place-visit';
  return null;
};

const directIdentityShape = (relation: PersonalAnchorRelation, text: string) => {
  const source = normalizeText(text);
  if (hardIdentityExclusion(relation, source)) return false;
  if (relation === 'home') {
...
const identityPassagesForNote = (
  ...
  return relations.flatMap(relation => {
    ...
    const exclusion = hardIdentityExclusion(relation, normalizedPassage);
    ...
    if (!negated && attribution !== 'third-party' && !exclusion) {
...
export type HostReviewedPassageResult = {
  accepted: boolean;
  reason: string;
  passage?: MemoryEvidencePassage;
};

export const buildHostReviewedEvidencePassage = ({
  note,
  star,
  relation,
  evidenceQuote,
  ...
  matchedTerms?: string[];
}): HostReviewedPassageResult => {
  const quote = String(evidenceQuote || '').trim().replace(/…$/u, '').trim();
  if (!quote || quote.length > MAX_PASSAGE_CHARACTERS) {
    return { accepted: false, reason: 'The evidence quote is empty or exceeds the passage limit.' };
  }
  const normalizedQuote = normalizeText(quote);
  const sourcePassage = notePassages(note).find(entry => normalizeText(entry.text).includes(normalizedQuote));
  if (!sourcePassage) {
    return { accepted: false, reason: 'The quoted text is not present in the candidate note.' };
  }
  const attribution = attributionFor(quote);
  if (negationPattern.test(quote)) return { accepted: false, reason: 'Negated text cannot establish memory evidence.' };
  if (attribution === 'third-party') return { accepted: false, reason: 'Third-party text cannot establish the user relation.' };
  if ((relation === 'home' || relation === 'work' || relation === 'study')) {
    if (attribution !== 'user') return { accepted: false, reason: 'A personal anchor requires a first-person passage.' };
    const exclusion = hardIdentityExclusion(relation, quote);
    if (exclusion) return { accepted: false, reason: `The passage was rejected as ${exclusion}.` };
  }
  return {
    accepted: true,
    reason: 'The host AI interpretation is bound to an exact authenticated note passage.',
    passage: {
      ...passageRecord(note, star, sourcePassage.source, quote, relation, 'corroborating', matchedTerms),
