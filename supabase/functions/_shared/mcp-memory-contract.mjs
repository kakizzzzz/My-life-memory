...
export const RESEARCH_MEMORY_TOOL_DESCRIPTION = 'Primary tool for natural-language questions about public places, dates, trips, routines, personal anchors such as home/work/study, observations, activities, nearby routes, and combinations of those constraints. Keep private user-relative relations in query and put only an explicit public geographic name in place. Deterministic evidence is tried first. If semanticReview.required is true, the AI application already handling the conversation may review the bounded candidate quotes and call this same tool once more with semanticReview.decisions; My Life Memory never calls a model service. answerBoundary is mandatory. Confidence is heuristic.';
...
const baseInstructions = [
  ...
  'My Life Memory does not contain, call, or pay for a model service. Only when semanticReview.required is true, the AI application already handling the conversation may inspect the bounded candidateNotes and call research_memory_context again with exact-quote decisions. If it cannot do that, state that evidence is insufficient.',
  'A semantic-review supports decision must quote candidate text exactly. Mark temporary lodging or hospitalization, visits, negated claims, third-party places, quotations about someone else, and uncertainty as rejects or uncertain.',
